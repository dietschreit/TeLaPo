...
CharmmPSF * ReadPSFfile(FILE *Inp, int *P_natom);
...
// Normal dcdplugin write_timestep
...
/* These are write-plugins written by Aysam and Jorge Numata


// Aysam: Polymer Free energy write_timestep for vector of vectors of Vec3
static int write_timestep(void *v, vector < vector < Vec3 > >& lcoord)
{
    dcdhandle *dcd = (dcdhandle *)v;
...
// Numata: write_timestep for alloweddcd and disalloweddcd
static int write_timestep(void *v, vector < vector < Vec3 > >& lcoord, vector <int>& conformations_alloweddcd)
{
    dcdhandle *dcd = (dcdhandle *)v;
    int i, rc, curstep, curindex;
    double unitcell[6];
    ...
    if (usealloweddcd==2) {allowordisallow=0;} // disallowed
    if (usealloweddcd!=0){
        for (curindex = 0; curindex < lcoord.size(); curindex++)
        {
...
/* This Initialization seems to be unneccessary at the moment
...
void dcd_to_polymer(double (**polymer), void *v, int natoms, molfile_timestep_t *ts, dcdhandle *dcd_hadle, unsigned long i,int numberofbeads){
    ...
    if (rc) {
        fprintf(stderr, "error in read_next_timestep on frame %lu\n", i);
        exit(1);
    }
    // polymer is set to start at origin
    for (int j=0; j<(numberofbeads); j++) {
        polymer[j][0]=(dcd_hadle->x[j] - dcd_hadle->x[0]);
